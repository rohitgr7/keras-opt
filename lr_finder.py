...
class LRFinder:
    ...
    def find(self, X, y, batch_size=32, epochs=100, start_lr=1e-3, end_lr=10):
        ...
        self.model.fit(X, y, batch_size=batch_size, epochs=epochs, callbacks=[callback])
    ...
    def plot_loss(self, n_skip_start=10, n_skip_end=5):
        plt.plot(self.lrs[n_skip_start:-(n_skip_end + 1)], self.losses[n_skip_start:-(n_skip_end + 1)])
        plt.xlabel('Learning rate (log scale)')
        plt.ylabel('Loss')
        plt.xscale('log')

    def plot_dloss(self, sma=1, n_skip_start=10, n_skip_end=5, y_lim=(-1e-2, 1e-2)):
        derivatives = [0.] * (sma)
        ...
        plt.plot(self.lrs[n_skip_start:-(n_skip_end + 1)], derivatives[n_skip_start:-(n_skip_end + 1)])
        plt.xlabel("Learning rate (log scale)")
        plt.ylabel("dLoss")
        plt.xscale('log')
        plt.ylim(y_lim)
